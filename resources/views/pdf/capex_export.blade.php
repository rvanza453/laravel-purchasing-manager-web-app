...
<body>
    ...
    @php
        ...
        $estateManager = match(true) {
            str_contains($deptUpper, 'PKS') => 'Iken Wahyudi', // If department contains PKS
            str_contains($deptUpper, 'MJE') => 'Ridwan',       // If department contains MJE
            str_contains($deptUpper, 'KDE') => 'Mulia',        // If department contains KDE
            default => $capex->user->name // Default to creator name if no match
        };
    @endphp
    ...
    <table class="bordered" style="width: 100%; margin-bottom: 10px;">
        ...
        <tr>
            <td>Estate Manager/Department Head :</td>
            <td>{{ $estateManager }}</td>
            <td>Date</td>
