            ...
            <form action="{{ route('departments.update', $department) }}" method="POST">
                ...
                <div class="mb-6 bg-blue-50 p-4 rounded-lg border border-blue-100">
                    ...
                    <select id="budget_type" name="budget_type" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500">
                        <option value="station" {{ $department->budget_type === 'station' ? 'selected' : '' }}>Per Station</option>
                        <option value="job_coa" {{ $department->budget_type === 'job_coa' ? 'selected' : '' }}>Per Pekerjaan (Job)</option>
                    </select>
