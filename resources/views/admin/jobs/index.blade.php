    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm p-4">
             <form action="{{ route('jobs.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4">
                 <div class="flex-1">
                     <x-text-input name="search" class="w-full" placeholder="Cari Kode atau Nama Job..." value="{{ request('search') }}" />
                 </div>
                 ...
                 @if(request()->has('search'))
                    <a href="{{ route('jobs.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-md text-sm font-semibold hover:bg-gray-200 transition text-center">Reset</a>
                 @endif
             </form>
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($jobs as $job)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $job->code }}</td>
