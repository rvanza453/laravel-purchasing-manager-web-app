<x-admin-layout>
    <div class="max-w-2xl">
        <div class="mb-6">
            ...
            <p class="mt-1 text-sm text-gray-600">Perbarui informasi block {{ $block->name }}</p>
        </div>
        ...
        <!-- Form -->
        <form method="POST" action="{{ route('admin.blocks.update', $block) }}" class="bg-white rounded-lg shadow">
            @csrf
            @method('PUT')
            <div class="p-6 space-y-6">
                            ...
                            <optgroup label="{{ $siteName }}">
                                @foreach ($deptGroup as $afdeling)
                                    <option value="{{ $afdeling->id }}" 
                                            @selected(old('sub_department_id', $block->sub_department_id) == $afdeling->id)>
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $block->name) }}" required
                ...
                <div>
                    ...
                    <input type="text" id="code" name="code" value="{{ old('code', $block->code) }}" required
