<section>
    ...
    @if($user->signature_path)
        <div class="mt-6">
            ...
            <form method="POST" action="{{ route('profile.signature.delete') }}" class="mt-3">
                @csrf
                @method('DELETE')
                <x-danger-button type="submit" onclick="return confirm('Hapus tanda tangan?')">
    ...
    <form method="POST" action="{{ route('profile.signature.upload') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
        ...
        <div>
            <x-input-label for="signature" :value="__('Upload Tanda Tangan Baru')" />
            <input id="signature" 
                   name="signature" 
                   type="file" 
        ...
        <div class="flex items-center gap-4">
            <x-primary-button>{{ __('Upload Tanda Tangan') }}</x-primary-button>
        </div>
    </form>
</section>
