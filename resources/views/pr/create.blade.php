<x-app-layout>
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select id="sub_department_id" name="sub_department_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500" required disabled>
    ...
    <script>
        ...
        const departmentsData = @json($departments); // Contains budget_type
        ...
        function updateSubDepartments() {
            ...
            // Reset Job Data
            availableJobs = [];
            currentBudgetType = BUDGET_TYPE_STATION; // Default
                ...
                if (selectedDept) {
                    ...
                    if (selectedDept.sub_departments && selectedDept.sub_departments.length > 0) {
                        ...
                        selectedDept.sub_departments.forEach(sub => {
                            subDeptSelect.innerHTML += `<option value="${sub.id}">${sub.name} (${sub.coa || '-'})</option>`;
                        });
                    } else {
                        subDeptSelect.disabled = true;
                        subDeptSelect.innerHTML = '<option value="">Tidak ada Stasiun / Afdeling</option>';
                    }
                }
            } else {
                subDeptSelect.disabled = true;
                subDeptSelect.innerHTML = '<option value="">Pilih Unit Terlebih Dahulu</option>';
            }
        ...
        function onSubDepartmentChange() {
             ...
             if(subId) {
                 ...
                 // 2. If Job COA mode, fetch Jobs
                 if (currentBudgetType === BUDGET_TYPE_JOB_COA) {
                     fetch(`/api/sub-department/${subId}/jobs`)
                        .then(r => r.json())
                        .then(jobs => {
                            availableJobs = jobs;
                            refreshJobDropdowns();
        ...
        function refreshJobDropdowns() {
            ...
            availableJobs.forEach(job => {
                html += `<option value="${job.id}">${job.code} - ${job.name}</option>`;
            });
        ...
        function onGlobalJobChange() {
            ...
            const jobVal = document.getElementById('global_job_id').value;
        ...
        function addItem() {
            ...
            // UPDATED ROW STRUCTURE: GRID YANG BENAR-BENAR MENYAMPING (SATU BARIS)
            const row = `
                <div class="grid grid-cols-12 gap-3 item-row p-4 border-b border-gray-100 items-start hover:bg-gray-50 transition duration-150 group" id="${rowId}">
                    ...
                    <div class="col-span-3">
                        ...
                        <div id="manual-name-container-${currentIndex}" class="mt-2 hidden space-y-2 p-2 bg-yellow-50 rounded-md border border-yellow-100">
                            ...
                            <input type="url" name="items[${currentIndex}][url_link]" placeholder="Link (Opsional)" class="block w-full border-gray-300 rounded-md text-sm focus:border-primary-500 focus:ring-primary-500">
                        </div>
                    </div>
                    ...
                    <div class="col-span-2">
                        <textarea name="items[${currentIndex}][remarks]" rows="1" class="block w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500 text-sm min-h-[38px] resize-none overflow-hidden leading-snug" placeholder="Ket..." oninput="this.style.height = ''; this.style.height = this.scrollHeight + 'px'"></textarea>
                    </div>
                 ...
                 if(qtyInput && priceInput) {
                     ...
                     if (currentBudgetType === BUDGET_TYPE_JOB_COA) {
                         ...
                         if (selectProduct && selectProduct.value === 'manual') {
                             ...
                             const p = products.find(x => x.id == selectProduct.value);
