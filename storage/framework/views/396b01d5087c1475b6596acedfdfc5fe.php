...
<?php $component = Modules\ServiceAgreementSystem\View\Components\Layouts\Master::resolve(['title' => 'Buat Pengajuan USPK'] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $component->withAttributes([]); ?>
    ...
    <?php $__env->startPush('actions'); ?>
        <a href="<?php echo e(route('sas.uspk.index')); ?>" class="btn btn-secondary btn-sm" style="border-radius: 8px; font-weight: 500;">
    ...
    <form action="<?php echo e(route('sas.uspk.store')); ?>" method="POST" enctype="multipart/form-data" id="uspkForm">
        <?php echo csrf_field(); ?>
        ...
        <div class="card modern-card">
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input type="hidden" name="title" id="titleInput" value="<?php echo e(old('title')); ?>">
                    <div id="customTitleWrapper" style="margin-top: 10px; display: none;">
                        <input type="text" id="customTitleInput" class="form-control" placeholder="Tuliskan nama kegiatan..." value="<?php echo e(old('title')); ?>">
                    </div>
                </div>

                <div class="form-group mb-0">
                    <label class="form-label">Deskripsi / Uraian Pekerjaan</label>
                    <textarea name="description" class="form-control" rows="3" placeholder="Jelaskan secara detail pekerjaan yang akan diborongkan..."><?php echo e(old('description')); ?></textarea>
                </div>
            </div>
        </div>
                            ...
                            <select name="job_id" id="job_id" class="form-control select2-searchable" required>
                                ...
                                <?php $__currentLoopData = $jobs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $job): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($job->id); ?>" <?php echo e(old('job_id') == $job->id ? 'selected' : ''); ?>>
                                        <?php echo e($job->code ? $job->code . ' - ' : ''); ?><?php echo e($job->name); ?>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="tenders[0][contractor_id]" class="form-control select2-searchable contractor-select" required>
                                        ...
                                        <?php $__currentLoopData = $contractors; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <option value="<?php echo e($c->id); ?>"><?php echo e($c->name); ?> <?php echo e($c->company_name ? '(' . $c->company_name . ')' : ''); ?></option>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                    </select>
                                </div>
                            </div>
        ...
        <div class="d-flex justify-content-end align-items-center mb-5" style="gap: 15px;">
            <a href="<?php echo e(route('sas.uspk.index')); ?>" class="btn text-danger" style="font-weight: 600;">Batal</a>
    ...
    <script>
        let tenderCount = 1;
        const maxTenders = 3;
        const contractorsData = <?php echo json_encode($contractors, 15, 512) ?>;
        const departmentsData = <?php echo json_encode($departments, 15, 512) ?>;
        const jobsData = <?php echo json_encode($jobs, 15, 512) ?>;
        
        const selectedBudgetId = <?php echo json_encode(old('uspk_budget_activity_id'), 15, 512) ?>;
        const initialDepartmentId = <?php echo json_encode(old('department_id'), 15, 512) ?>;
        const initialJobId = <?php echo json_encode(old('job_id'), 15, 512) ?>;
        const initialSubDepartmentId = <?php echo json_encode(old('sub_department_id'), 15, 512) ?>;
        const initialBlockIds = (<?php echo json_encode(old('block_ids', []), 512) ?> || []).map(String);
        ...
        const oldTitle = <?php echo json_encode(old('title', ''), 512) ?>;
        if (oldTitle) {
...
<?php endif; ?><?php /**PATH C:\laragon\www\plantation.oilpam.my.id\Modules/ServiceAgreementSystem\resources/views/uspk/create.blade.php ENDPATH**/ ?>