...
class InventoryImportController extends Controller
{
    public function formOut()
    {
        ...
        return view('inventory.import_out', compact('warehouses', 'departments'));
    }
        ...
        if (!empty($coaFull)) {
            ...
            if (count($parts) >= $segmentsToKeep) {
                ...
                if (!empty($linkedDeptIds)) {
                    $department = \App\Models\Department::whereIn('id', $linkedDeptIds)->first();
                }
                ...
                // ========================================
                if ($department && $department->budget_type === \App\Enums\BudgetingType::STATION) {
                ...
                if (!$budgetDeducted) {
                    ...
                    if (!$job && (str_starts_with($codeToCheck, '1') || str_starts_with($codeToCheck, '6'))) {
                        $traksiRoDepts = \App\Models\Department::whereIn('name', ['Traksi', 'RO'])
                            ->where('site_id', $defaultWarehouse->site_id)
                            ->pluck('id')
                            ->toArray();
                        ...
                        if ($budget) {
                            ...
                            if ($cost > 0) {
                                ...
                                if ($job->code === '5.3.1.04') {
                                    $sipilDept = \App\Models\Department::where('name', 'Sipil')
                                        ->where('site_id', $defaultWarehouse->site_id)
                                        ->first();
    ...
    $processed++;
                ...
                \App\Helpers\ActivityLogger::log('imported', "Imported Stock Data. Processed: $processed. StockUpd: $stockUpdated. BudgetUpd: $budgetUpdated", $defaultWarehouse);
