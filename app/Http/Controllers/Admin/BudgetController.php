...
class BudgetController extends Controller
{
    public function index(Request $request)
    {
        ...
        $sites = \App\Models\Site::with(['departments', 'departments.budgets' => function($q) use ($year) {
                $q->where('year', $year);
    ...
    public function editDepartment(\App\Models\Department $department)
    {
        ...
        $jobs = \App\Models\Job::where('site_id', $department->site_id)
        ...
        return view('admin.budget.edit', compact('department', 'existingBudgets', 'isJobCoa', 'jobs'));
    }
    ...
    public function monitoring(Request $request)
    {
        ...
        $user = auth()->user();
        $isHO = $user->hasRole('Admin') || \App\Models\GlobalApproverConfig::where('user_id', $user->id)->exists();
        
        if (!$isHO && $user->site_id) {
            // Unit Approver: Restricted to their Site
            $request->merge(['site_id' => $user->site_id]);
            $site_id = $user->site_id;
            $sites = \App\Models\Site::where('id', $site_id)->get();
        } else {
        ...
        return view('admin.budget.monitoring', compact('budgets', 'sites', 'departments', 'year', 'site_id', 'department_id', 'chartData'));
    }
    ...
    public function usageDetails(Budget $budget)
    {
        ...
        if ($budget->job_id) {
            $movements = \App\Models\StockMovement::where('job_id', $budget->job_id)
            ...
            $movements = \App\Models\StockMovement::where('sub_department_id', $budget->sub_department_id)
                ->whereYear('date', $budget->year)
                ->with(['product', 'warehouse'])
                ->orderBy('date', 'desc')
                ->get();
        ...
        return response()->json([
            'html' => view('admin.budget.components.usage_detail_modal', compact('movements', 'budget'))->render()
        ]);
    }
}
