...
class InventoryController extends Controller
{
    ...
    {
        $sites = Site::all();
        return view('inventory.create', compact('sites'));
    }
    ...
    {
        $sites = Site::all();
        return view('inventory.create', compact('warehouse', 'sites'));
    }
    ...
    public function destroy(Request $request, Warehouse $warehouse)
    {
        ...
        $name = $warehouse->name;
        $warehouse->delete();
        \App\Helpers\ActivityLogger::log('deleted', 'Deleted Warehouse: ' . $name);
    ...
    public function show(Request $request, Warehouse $warehouse)
    {
        ...
        return view('inventory.show', compact('warehouse', 'stocks', 'movements'));
    }
    ...
    public function createMovement(Warehouse $warehouse, $type)
    {
        ...
        $departments = \App\Models\Department::with('subDepartments')->orderBy('name')->get(); 
        
        // Filter Jobs by the Warehouse's Site
        $jobs = \App\Models\Job::where('site_id', $warehouse->site_id)
                    ->orderBy('code')
                    ->get();
        
        return view('inventory.movement', compact('warehouse', 'type', 'products', 'departments', 'jobs'));
    }
    ...
    public function storeMovement(Request $request, Warehouse $warehouse)
    {
        ...
        try {
            DB::transaction(function () use ($request, $warehouse) {
                foreach ($request->items as $item) {
                    ...
                    if ($request->type === 'OUT' && $cost > 0) {
                         $subDept = \App\Models\SubDepartment::find($item['sub_department_id'] ?? null);
                         ...
                         if ($subDept) {
                             ...
                             if ($subDept->department->budget_type === \App\Enums\BudgetingType::JOB_COA) {
            ...
            \App\Helpers\ActivityLogger::log('movement', 'Recorded Inventory Movement: ' . $request->type . ' (' . count($request->items) . ' items)', $warehouse);
