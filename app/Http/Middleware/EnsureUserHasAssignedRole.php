...
class EnsureUserHasAssignedRole
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $isLeavingImpersonation = $request->routeIs('users.leave-impersonate')
            || $request->routeIs('admin.users.leave-impersonate');

        if ($isLeavingImpersonation && $request->session()->has('impersonate_admin_id')) {
            return $next($request);
        }
