...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-1">
        <div class="glass-panel p-6 sticky top-24">
            ...
            <form action="{{ route('systemsupport.announcements.store') }}" method="POST">
                @csrf
                <div class="space-y-4">
                    <div>
    ...
    <div class="lg:col-span-2 space-y-4">
        @forelse($announcements as $ann)
            <div class="glass-panel p-5 relative overflow-hidden flex flex-col sm:flex-row justify-between items-start gap-4">
                ...
                @endphp
                <div class="absolute left-0 top-0 bottom-0 w-1 {{ $colors[$ann->type] ?? 'bg-gray-500' }}"></div>
                
                <div class="flex-1 w-full pl-2">
                    <div class="flex items-center gap-2 mb-2">
                        <span class="text-[10px] uppercase font-bold px-2 py-0.5 rounded border {{ $bgColors[$ann->type] ?? 'bg-gray-800 text-gray-400 border-gray-600' }}">
                            {{ $ann->type }}
                        </span>
                    ...
                    <h4 class="text-lg font-bold text-white mb-1">{{ $ann->title }}</h4>
                    <p class="text-sm text-gray-400 mb-3 leading-relaxed">{{ Str::limit($ann->content, 120) }}</p>
                    
                    <div class="flex flex-wrap items-center gap-4 text-xs font-mono text-gray-500">
                        <span><i class="fa-regular fa-clock mr-1"></i> Created {{ $ann->created_at->diffForHumans() }}</span>
                    </div>
                </div>
                
                <div class="flex sm:flex-col gap-2 w-full sm:w-auto mt-4 sm:mt-0 pt-4 sm:pt-0 border-t sm:border-none border-[#30363d]">
                    <form action="{{ route('systemsupport.announcements.toggle', $ann->id) }}" method="POST">
                        @csrf
                        <button type="submit" class="w-full sm:w-28 text-center text-xs font-bold py-2 rounded-lg border {{ $ann->is_active ? 'border-yellow-500/50 text-yellow-500 hover:bg-yellow-500/10' : 'border-green-500/50 text-green-500 hover:bg-green-500/10' }} transition">
                            <i class="fa-solid fa-power-off mr-1"></i> {{ $ann->is_active ? 'Turn Off' : 'Turn On' }}
                        </button>
                    </form>
                    
                    <form action="{{ route('systemsupport.announcements.destroy', $ann->id) }}" method="POST" onsubmit="return confirm('Hapus permanen pengumuman ini?');">
                        @csrf
                        @method('DELETE')
