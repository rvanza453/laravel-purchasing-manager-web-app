    ...
    <div class="max-w-7xl mx-auto space-y-6">
        ...
        @if($pr->isReopened())
            <div class="bg-purple-50 border border-purple-200 rounded-xl p-4 flex items-center gap-3">
                ...
                <div>
                    <h4 class="text-sm font-bold text-purple-900">PR Dibuka Kembali (Reopened)</h4>
                    <p class="text-xs text-purple-700 mt-1">
                        PR ini sebelumnya telah kadaluarsa dan dibuka kembali oleh <strong>{{ $pr->reopenedBy?->name ?? 'Admin' }}</strong> pada <strong>{{ $pr->reopened_at?->format('d M Y H:i') }}</strong>.
                    </p>
                </div>
            </div>
        @endif
                ...
                <div>
                    ...
                    @if($pr->attachment_path)
                         <div class="flex items-center gap-2 mt-1">
                                      <a href="{{ route('pr.attachment.view', $pr) }}" target="_blank" class="inline-flex items-center gap-1 px-3 py-1.5 text-sm font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-lg transition-colors">
            ...
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                ...
                @if($isApproved)
                    @if($isExpired)
                        ...
                        @endphp
                        <div class="flex items-center gap-3">
                            <div class="px-3 py-1 {{ $expiredClass }} rounded-lg text-sm font-bold border flex items-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                                @if($pr->po_status === 'Partial PO')
                            ...
                            @if($isPrAdmin)
                                <form action="{{ route('pr.reopen-expired', $pr) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin membuka kembali PR ini agar dapat diproses PO?');">
                                    @csrf
                                    <button type="submit" class="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 text-sm font-bold shadow-sm transition inline-flex items-center gap-2">
                            ...
                            @if($canApprove)
                                <td class="px-6 py-4 text-sm text-gray-900 bg-blue-50">
                                    ...
                                    <input type="number" 
                                           ...
                                           min="0" 
                                           step="1"
                                           value="{{ old('adjusted_quantities.' . $item->id) }}"
                                           placeholder="{{ $finalQty }}">
                                </td>
                            @else
                                <td class="px-6 py-4 text-sm text-gray-900">
                                    @php
