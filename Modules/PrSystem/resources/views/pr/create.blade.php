<x-prsystem::app-layout>
    <div class="max-w-7xl mx-auto py-6">
            ...
            <form action="{{ route('pr.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                     ...
                     <div id="budget-summary-section" class="hidden mt-4 border border-blue-200 bg-blue-50/60 rounded-xl p-4">
                         <div class="flex items-center justify-between gap-3 mb-3">
                             <div>
                                 <h4 class="text-sm font-bold text-blue-900">Ringkasan Budget</h4>
                                 <p class="text-xs text-blue-700">Limit sudah termasuk PTA bila ada.</p>
                             </div>
                             <div class="text-xs text-blue-700 font-semibold" id="budget-summary-status">Memuat data...</div>
                         </div>
                         <div class="overflow-x-auto">
                             <table class="min-w-full text-sm bg-white rounded-lg overflow-hidden border border-blue-100">
                                 <thead class="bg-blue-100">
                                     <tr>
                                         <th class="px-3 py-2 text-left font-semibold text-blue-900">Budget</th>
                                         <th class="px-3 py-2 text-right font-semibold text-blue-900">Limit</th>
                                         <th class="px-3 py-2 text-right font-semibold text-blue-900">Terpakai</th>
                                         <th class="px-3 py-2 text-right font-semibold text-blue-900">Sisa</th>
                                     </tr>
                                 </thead>
                                 <tbody id="budget-summary-body" class="divide-y divide-blue-50">
                                     <tr>
                                         <td colspan="4" class="px-3 py-4 text-center text-blue-700">Pilih Stasiun / Afdeling untuk melihat budget.</td>
                                     </tr>
                                 </tbody>
                             </table>
                         </div>
     ...
     <script>
         ...
         const budgetSummaryStatus = document.getElementById('budget-summary-status');
         ...
         function formatRupiah(amount) {
             return 'Rp ' + new Intl.NumberFormat('id-ID', { maximumFractionDigits: 0 }).format(amount || 0);
         }

         function renderBudgetSummary() {
             if (!budgetSummarySection || !budgetSummaryBody || !budgetSummaryStatus) {
                 return;
             }

             const entries = Object.entries(budgetData || {});
             if (!subDeptSelect.value || entries.length === 0) {
                 budgetSummarySection.classList.add('hidden');
                 budgetSummaryStatus.textContent = 'Belum ada data';
                 budgetSummaryBody.innerHTML = '<tr><td colspan="4" class="px-3 py-4 text-center text-blue-700">Pilih Stasiun / Afdeling untuk melihat budget.</td></tr>';
                 return;
             }

             budgetSummarySection.classList.remove('hidden');
             budgetSummaryStatus.textContent = `${entries.length} budget ditemukan`;

             budgetSummaryBody.innerHTML = entries.map(([label, data]) => {
                 ...
                 return `
                     <tr>
                         <td class="px-3 py-2 text-gray-800 font-medium">${label}</td>
                         <td class="px-3 py-2 text-right text-gray-700">${formatRupiah(limit)}</td>
                         <td class="px-3 py-2 text-right text-gray-700">${formatRupiah(used)}</td>
                         <td class="px-3 py-2 text-right font-semibold ${remainingClass}">${formatRupiah(remaining)}</td>
                     </tr>
                 `;
             }).join('');
         }

         async function loadBudgetStatus(targetId = null) {
             const budgetTargetId = targetId || (currentBudgetType === BUDGET_TYPE_JOB_COA ? deptSelect.value : subDeptSelect.value);
             ...
             if (budgetSummaryStatus) {
                 budgetSummaryStatus.textContent = 'Memuat data...';
             }

             try {
                 const response = await fetch(`/api/budget/${budgetTargetId}?scope=${currentBudgetType}`);
