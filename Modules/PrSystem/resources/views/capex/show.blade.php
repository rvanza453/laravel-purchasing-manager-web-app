<x-prsystem::app-layout>
    @php
        $hasWetFile = filled($capex->signed_file_path);
    @endphp
                    ...
                    <div>
                        @if($capex->supporting_document_path)
                            <div class="flex items-center gap-2">
                                <a href="{{ route('capex.supporting-file.view', $capex) }}" target="_blank" class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 rounded-md transition">
                                    Lihat PDF
                                </a>
                                <a href="{{ route('capex.supporting-file.download', $capex) }}" class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded-md transition">
                                    Download PDF
                                </a>
                            </div>
                        @else
                        ...
                        <div class="flex-1">
                            ...
                            @if($hasWetFile)
                                <div class="mt-2 flex flex-wrap gap-2">
                                                <a href="{{ route('capex.signed-file.view', $capex) }}" target="_blank"
                                       class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-green-50 text-green-700 border border-green-200 rounded-lg hover:bg-green-100 transition">
                                        ...
                                        View File
                                                ...
                                                <a href="{{ route('capex.signed-file.download', $capex) }}"
                        ...
                        <div class="flex-shrink-0">
                            ...
                            @elseif($allDigitalDone)
                                @if (auth()->user()->hasRole('Admin'))
                                    @if($hasWetFile)
                                        <form action="{{ route('capex.upload', $capex) }}" method="POST" enctype="multipart/form-data" class="flex flex-col items-start gap-2">
                                            @csrf
                                            <input type="file" name="signed_file" accept=".pdf,.jpg,.jpeg,.png" class="block w-72 text-sm text-gray-700 border border-yellow-300 rounded-lg bg-white px-3 py-2" required>
                                            <button type="submit" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-yellow-50 text-yellow-700 border border-yellow-300 rounded-lg hover:bg-yellow-100 transition">
                                    ...
                                    @else
                                        <form action="{{ route('capex.upload', $capex) }}" method="POST" enctype="multipart/form-data" class="flex flex-col items-start gap-2">
                                            @csrf
                                            <input type="file" name="signed_file" accept=".pdf,.jpg,.jpeg,.png" class="block w-72 text-sm text-gray-700 border border-indigo-300 rounded-lg bg-white px-3 py-2" required>
                                            <button type="submit" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition shadow-sm">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
