<x-prsystem::app-layout>
        ...
        <form method="POST" action="{{ route('po.update', $po) }}" id="po-form">
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 space-y-6">
                ...
                <div class="border-t border-gray-200 pt-6">
                    ...
                    <div class="flex flex-col md:flex-row justify-between md:items-center gap-4 bg-gray-50 p-4 rounded-lg border border-gray-200 mb-4">
                        <div class="w-full md:flex-1">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Pilih Vendor/Supplier</label>
                            <select id="vendor-select" name="vendor_id" class="w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500" onchange="handleVendorChange()">
                                <option value="">-- Gunakan data vendor PO saat ini --</option>
                                @foreach($vendors as $vendor)
                                    <option value="{{ $vendor->id }}"
                                        {{ (string) old('vendor_id', $po->vendor_id) === (string) $vendor->id ? 'selected' : '' }}
    ...
    <script>
        ...
        function handleVendorChange() {
            ...
            document.getElementById('vendor_postal_code').value = document.getElementById('vendor_postal_code').value || '';

            const manualToggle = document.getElementById('manual-edit-toggle');
            if (manualToggle) {
