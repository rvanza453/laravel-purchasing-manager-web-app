...
// PR-specific activity logs route that works with pr.role:Admin
Route::middleware(['auth', 'assigned.role', 'pr.role:Admin'])->group(function () {
    Route::get('/activity-logs', [\Modules\PrSystem\Http\Controllers\Admin\ActivityLogController::class, 'index'])->name('activity-logs.index');
});
...
Route::middleware(['auth', 'assigned.role', 'pr.role'])->group(function () {
    ...
    Route::get('/pr/{purchaseRequest}/attachment/view', [PrController::class, 'viewAttachment'])->name('pr.attachment.view');
    ...
    Route::middleware(['pr.role:Admin'])->group(function () {
        ...
        Route::get('/inventory-import/kde-script', [\Modules\PrSystem\Http\Controllers\InventoryImportController::class, 'showInitialImportForm'])->name('inventory.import.kde');
        Route::post('/inventory-import/kde-script', [\Modules\PrSystem\Http\Controllers\InventoryImportController::class, 'importKdeInventory'])->name('inventory.import.kde.process');
    ...
    Route::get('/capex/{capex}/signed-file/view', [\Modules\PrSystem\Http\Controllers\CapexController::class, 'viewSignedFile'])->name('capex.signed-file.view');
    Route::get('/capex/{capex}/signed-file/download', [\Modules\PrSystem\Http\Controllers\CapexController::class, 'downloadSignedFile'])->name('capex.signed-file.download');
    Route::get('/capex/{capex}/supporting-file/view', [\Modules\PrSystem\Http\Controllers\CapexController::class, 'viewSupportingDocument'])->name('capex.supporting-file.view');
    Route::get('/capex/{capex}/supporting-file/download', [\Modules\PrSystem\Http\Controllers\CapexController::class, 'downloadSupportingDocument'])->name('capex.supporting-file.download');
