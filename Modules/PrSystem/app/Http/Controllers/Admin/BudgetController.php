...
class BudgetController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($site_id) {
            ...
            $departments->each(function($dept) use ($year) {
                if ($dept->budget_type === \Modules\PrSystem\Enums\BudgetingType::JOB_COA) {
                    $dept->total_budget = $dept->budgets->sum(fn ($budget) => $budget->totalAmount());
                } else {
                    $dept->total_budget = $dept->subDepartments->flatMap->budgets->sum(fn ($budget) => $budget->totalAmount());
                }
            });
        ...
        $sites->each(function($site) {
            $site->total_budget = $site->departments->sum(function($dept) {
                if ($dept->budget_type === \Modules\PrSystem\Enums\BudgetingType::JOB_COA) {
                    return $dept->budgets->sum(fn ($budget) => $budget->totalAmount());
                } else {
                    return $dept->subDepartments->flatMap->budgets->sum(fn ($budget) => $budget->totalAmount());
                }
            });
    ...
    public function edit(SubDepartment $subDepartment)
    {
        ...
        $existingPtaBudgets = $subDepartment->budgets->pluck('pta_amount', 'category')->toArray();
        $jobs = [];

        return view('prsystem::admin.budget.edit', compact('subDepartment', 'categories', 'existingBudgets', 'existingPtaBudgets', 'isJobCoa', 'jobs'));
    }
    ...
    public function update(Request $request, SubDepartment $subDepartment)
    {
        $request->validate([
            ...
            'pta_budgets' => 'array',
            'pta_budgets.*' => 'nullable|numeric|min:0',
        ]);
        ...
        foreach ($request->budgets as $category => $amount) {
            ...
            $budget->pta_amount = $request->input("pta_budgets.$category", $budget->pta_amount ?? 0);
            $budget->job_id = null;
    ...
    public function editDepartment(\Modules\PrSystem\Models\Department $department)
    {
        ...
        $existingPtaBudgets = $department->budgets->pluck('pta_amount', 'job_id')->toArray();
        $isJobCoa = true;

        return view('prsystem::admin.budget.edit', compact('department', 'existingBudgets', 'existingPtaBudgets', 'isJobCoa', 'jobs'));
    }
    ...
    public function updateDepartment(Request $request, \Modules\PrSystem\Models\Department $department)
    {
        ...
        foreach ($request->budgets as $jobId => $amount) {
            ...
            $budget->amount = $amount ?? 0;
            $budget->pta_amount = $request->input("pta_budgets.$jobId", $budget->pta_amount ?? 0);
    ...
    public function monitoring(Request $request)
    {
        ...
        $isHO = $user->hasRole('Admin') || $user->hasPrGlobalApproverAllSitesScope();
        
        if (!$isHO && !empty($globalApproverSiteIds)) {
            $site_id = $request->input('site_id');
            if (!$site_id || !in_array((int) $site_id, $globalApproverSiteIds, true)) {
                $site_id = $globalApproverSiteIds[0];
            }

            $request->merge(['site_id' => $site_id]);
            $sites = \Modules\PrSystem\Models\Site::whereIn('id', $globalApproverSiteIds)->get();
        } elseif (!$isHO && $user->site_id) {
        ...
        $query = Budget::with(['subDepartment', 'job', 'department', 'subDepartment.department.site', 'department.site'])
            ...
            ->where(function ($q) {
                // Show if Budget exists OR if it has been Used (even if budget is 0)
                                $q->whereRaw('(COALESCE(amount, 0) + COALESCE(pta_amount, 0)) > 0')
        ...
        if (!$site_id) {
            ...
            foreach ($grouped as $name => $items) {
                ...
                if ($id) {
                    ...
                    $chartData['budget'][] = $items->sum(fn ($budget) => $budget->totalAmount());
            ...
            foreach ($budgets as $b) {
                ...
                $chartData['labels'][] = $name;
                $chartData['budget'][] = $b->totalAmount();
    ...
    public function addPta(Request $request, Budget $budget)
    {
        $validated = $request->validate([
            'pta_amount' => 'required|numeric|min:1',
        ]);

        $budget->increment('pta_amount', $validated['pta_amount']);

        return redirect()->route('admin.budgets.index')->with('success', 'PTA (Tambahan Anggaran) berhasil ditambahkan.');
    }
}
