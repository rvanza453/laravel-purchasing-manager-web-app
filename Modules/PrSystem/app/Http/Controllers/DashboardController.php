...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        if ($user->site && $user->site->code === 'HO') {
            $isHO = true;
        } elseif ($user->hasPrGlobalApproverAllSitesScope()) {
            $isHO = true;
        ...
        if (!$isHO) {
            if (!empty($globalApproverSiteIds)) {
                $statsQuery->whereHas('department', function ($q) use ($globalApproverSiteIds) {
                    $q->whereIn('site_id', $globalApproverSiteIds);
                });
            } elseif ($isApprover || $isPurchasing) {
        ...
        if ($isApprover || $isHO) {
            ...
            if (!$isHO) {
                if (!empty($globalApproverSiteIds)) {
                    $query->whereHas('purchaseRequest.department', function ($q) use ($globalApproverSiteIds) {
                        $q->whereIn('site_id', $globalApproverSiteIds);
                    });
                } else {
        ...
        if (!$isHO) {
            if (!empty($globalApproverSiteIds)) {
                $poCompletedQuery->whereHas('items.prItem.purchaseRequest.department', function($q) use ($globalApproverSiteIds) {
                    $q->whereIn('site_id', $globalApproverSiteIds);
                });
            } elseif ($isApprover) {
        ...
        if (!$isHO) {
            if (!empty($globalApproverSiteIds)) {
                $chartQuery->whereIn('departments.site_id', $globalApproverSiteIds);
            } else {
        ...
        if (!$isHO) {
            if (!empty($globalApproverSiteIds)) {
                $deptQuery->whereIn('site_id', $globalApproverSiteIds);
            } else {
        ...
        $departmentBudgets = $departments->map(function ($dept) use ($currentYear, $isHO) {
            ...
            // Calculate Allocated
            $allocated = $validBudgets->sum(fn ($budget) => $budget->totalAmount());
