...
class CapexController extends Controller
{
    ...
    public function store(\Illuminate\Http\Request $request)
    {
        ...
        if ($request->hasFile('supporting_document')) {
            ...
            if (!$supportingDocumentPath || !\Illuminate\Support\Facades\Storage::disk('public')->exists($supportingDocumentPath)) {
                return back()->withInput()->with('error', 'Upload dokumen pendukung gagal disimpan di server.');
            }
        }
    ...
    public function print(\Illuminate\Http\Request $request, \Modules\PrSystem\Models\CapexRequest $capex)
    {
        $signedFileAction = (string) $request->query('signed_file_action', '');
        if ($signedFileAction === 'view') {
            return $this->viewSignedFile($capex);
        }
        if ($signedFileAction === 'download') {
            return $this->downloadSignedFile($capex);
        }

        $supportingFileAction = (string) $request->query('supporting_file_action', '');
        if ($supportingFileAction === 'view') {
            return $this->viewSupportingDocument($capex);
        }
        if ($supportingFileAction === 'download') {
            return $this->downloadSupportingDocument($capex);
        }
    ...
    {
        $phpUploadError = (int) data_get($_FILES, 'signed_file.error', UPLOAD_ERR_NO_FILE);
        if ($phpUploadError !== UPLOAD_ERR_OK) {
            return back()->with('error', $this->mapPhpUploadError($phpUploadError));
        }

        $request->validate([
            'signed_file' => 'required|file|mimes:pdf,jpg,jpeg,png|max:20480',
        ]);

        if (! $request->hasFile('signed_file')) {
            return back()->with('error', 'File tidak terdeteksi. Silakan pilih file lalu upload kembali.');
        }

        $file = $request->file('signed_file');
        if (! $file->isValid()) {
            return back()->with('error', 'Upload gagal: file tidak valid atau koneksi terputus saat upload.');
        }

        try {
            $extension = strtolower((string) $file->getClientOriginalExtension());
            $safeCapexNumber = str_replace(['/', '\\'], '_', (string) $capex->capex_number);
            if ($safeCapexNumber === '') {
                $safeCapexNumber = 'capex-' . $capex->id;
            }

            $folderPath = 'capex_signed/' . $safeCapexNumber;
            \Illuminate\Support\Facades\Storage::disk('public')->makeDirectory($folderPath);

            $fileName = 'capex-signed-' . $safeCapexNumber . ($extension ? '.' . $extension : 'pdf');
            $path = $file->storeAs($folderPath, $fileName, 'public');

            if (!$path || !\Illuminate\Support\Facades\Storage::disk('public')->exists($path)) {
                return back()->with('error', 'Upload dokumen tanda tangan gagal disimpan di server.');
            }

            $capex->update(['signed_file_path' => $path]);

            return redirect()
                ->route('capex.show', $capex)
                ->with('success', 'Dokumen TTD basah berhasil diupload.');
        } catch (\Throwable $e) {
            report($e);
            return back()->with('error', 'Upload gagal diproses server. Silakan coba lagi.');
        }

    }

    private function mapPhpUploadError(int $errorCode): string
    {
        $uploadMax = (string) ini_get('upload_max_filesize');
        $postMax = (string) ini_get('post_max_size');

        return match ($errorCode) {
            UPLOAD_ERR_INI_SIZE => 'Upload gagal: ukuran file melebihi batas server (upload_max_filesize=' . $uploadMax . ', post_max_size=' . $postMax . ').',
            UPLOAD_ERR_FORM_SIZE => 'Upload gagal: ukuran file melebihi batas form HTML (MAX_FILE_SIZE).',
            UPLOAD_ERR_PARTIAL => 'Upload gagal: file hanya terunggah sebagian. Silakan ulangi.',
            UPLOAD_ERR_NO_FILE => 'File belum dipilih. Silakan pilih file terlebih dahulu.',
            UPLOAD_ERR_NO_TMP_DIR => 'Upload gagal: folder temporary server tidak tersedia.',
            UPLOAD_ERR_CANT_WRITE => 'Upload gagal: server tidak bisa menulis file ke disk.',
            UPLOAD_ERR_EXTENSION => 'Upload dihentikan oleh ekstensi PHP di server.',
            default => 'Upload gagal dengan kode error ' . $errorCode . '.',
        };
    }
    ...
    public function viewSignedFile(\Modules\PrSystem\Models\CapexRequest $capex)
    {
        $resolvedPath = $this->resolveSignedFilePath($capex);
        if (!$resolvedPath) {
            return back()->with('error', 'File TTD basah tidak ditemukan di server.');
        }

        return response()->file($resolvedPath);
    }

    public function downloadSignedFile(\Modules\PrSystem\Models\CapexRequest $capex)
    {
        $resolvedPath = $this->resolveSignedFilePath($capex);
        if (!$resolvedPath) {
            return back()->with('error', 'File TTD basah tidak ditemukan di server.');
        ...
        $safeCapexNumber = str_replace('/', '-', (string) $capex->capex_number);
        $downloadName = 'capex-signed-' . $safeCapexNumber . ($extension ? '.' . $extension : '');

        return response()->download($resolvedPath, $downloadName);
    }

    public function viewSupportingDocument(\Modules\PrSystem\Models\CapexRequest $capex)
    {
        $resolvedPath = $this->resolveCapexStoredFilePath((string) $capex->supporting_document_path, 'capex_supporting_documents');
        if (!$resolvedPath) {
            return back()->with('error', 'Dokumen pendukung tidak ditemukan di server.');
        }

        return response()->file($resolvedPath);
    }

    public function downloadSupportingDocument(\Modules\PrSystem\Models\CapexRequest $capex)
    {
        $resolvedPath = $this->resolveCapexStoredFilePath((string) $capex->supporting_document_path, 'capex_supporting_documents');
        if (!$resolvedPath) {
            return back()->with('error', 'Dokumen pendukung tidak ditemukan di server.');
        ...
        $extension = pathinfo($resolvedPath, PATHINFO_EXTENSION);
        $safeCapexNumber = str_replace('/', '-', (string) $capex->capex_number);
        $downloadName = 'capex-supporting-' . $safeCapexNumber . ($extension ? '.' . $extension : '');

        return response()->download($resolvedPath, $downloadName);
    }

    private function resolveSignedFilePath(\Modules\PrSystem\Models\CapexRequest $capex): ?string
    {
        return $this->resolveCapexStoredFilePath((string) $capex->signed_file_path, 'capex_signed');
    }

    private function resolveCapexStoredFilePath(string $rawPath, ?string $fallbackFolder = null): ?string
    {
        $raw = trim($rawPath);
        if ($raw === '') {
            return null;
        ...
        $searchBasename = basename($withoutPublicPrefix);
        $searchRoots = array_values(array_unique(array_filter([
            $fallbackFolder,
            ($normalized !== $searchBasename && dirname($normalized) !== '.') ? dirname($normalized) : null,
            ($withoutPublicPrefix !== $searchBasename && dirname($withoutPublicPrefix) !== '.') ? dirname($withoutPublicPrefix) : null,
        ])));
        ...
        foreach ($searchRoots as $root) {
            foreach (['public', 'local'] as $diskName) {
                ...
                $rootPath = $diskName === 'public'
                    ? storage_path('app/public/' . $root)
                    : storage_path('app/' . $root);
                ...
                foreach ($disk->allFiles($root) as $file) {
                    if (basename($file) === $searchBasename) {
