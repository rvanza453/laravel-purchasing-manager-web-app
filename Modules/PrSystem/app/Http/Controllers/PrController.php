...
class PrController extends Controller
{
    ...
    public function index(\Illuminate\Http\Request $request)
    {
        ...
        $hasGlobalAllSitesScope = $user->hasPrGlobalApproverAllSitesScope();
        ...
        $applyVisibility = function ($q) use ($isGlobal, $user, $globalApproverSiteIds) {
            if ($isGlobal) {
                return; // No filter
            }
            if (!empty($globalApproverSiteIds)) {
                $q->whereHas('department', function ($d) use ($globalApproverSiteIds) {
                    $d->whereIn('site_id', $globalApproverSiteIds);
                });
        ...
        if ($viewMode === 'items') {
            ...
            if ($sort === 'expired') {
                $query->join('purchase_requests', 'pr_items.purchase_request_id', '=', 'purchase_requests.id')
                      ->select('pr_items.*')
                      ->whereIn('purchase_requests.status', ['Approved', 'PO Created'])
                      ->whereHas('purchaseRequest.items', function ($iq) {
                          $iq->whereDoesntHave('poItems');
                      })
                ...
                $query->orderByRaw("COALESCE((" . $approvedAtSubquery->toSql() . "), '9999-12-31 23:59:59') ASC")
                ->addBinding($approvedAtSubquery->getBindings(), 'order');

            } elseif ($sort === 'expired_only') {
                $query->join('purchase_requests', 'pr_items.purchase_request_id', '=', 'purchase_requests.id')
                      ->select('pr_items.*')
                      ->whereIn('purchase_requests.status', ['Approved', 'PO Created'])
                      ->whereHas('purchaseRequest.items', function ($iq) {
                ...
                $query->orderByRaw("COALESCE((" . $approvedAtSubquery->toSql() . "), '1000-01-01 00:00:00') ASC")
                ->addBinding($approvedAtSubquery->getBindings(), 'order');
            ...
            if ($request->filled('search')) {
                ...
                if ($request->filled('status')) {
                    if ($request->status === \Modules\PrSystem\Enums\PrStatus::PENDING->value) {
                        ...
                        // Expired and Partial Expired PRs
                        $query->whereIn('status', ['Approved', 'PO Created'])
                              ->whereHas('items', function ($iq) {
                                  $iq->whereDoesntHave('poItems');
                              })
            ...
            $sort = $request->get('sort', 'terbaru');
            if ($sort === 'expired') {
                $query->whereIn('purchase_requests.status', ['Approved', 'PO Created'])
                      ->whereHas('items', function ($iq) {
                          $iq->whereDoesntHave('poItems');
                      })
                      ->whereExists(function($q) {
        ...
        if ($viewMode === 'pr') {
            $scopeKey = $isGlobal
                ? 'global'
                : (!empty($globalApproverSiteIds) ? ('scoped_' . implode('_', $globalApproverSiteIds)) : (string) $user->site_id);
            $cacheKey = 'pr_current_approvers_' . $user->id . '_' . $scopeKey;
            
            $approvers = \Cache::remember($cacheKey, 300, function () use ($user, $isGlobal, $globalApproverSiteIds) {
                ...
                if (!$isGlobal) {
                    if (!empty($globalApproverSiteIds)) {
                        $query->join('departments as d', 'pr.department_id', '=', 'd.id')
                            ->whereIn('d.site_id', $globalApproverSiteIds);
                    } elseif ($user->hasRole(['Purchasing', 'Approver'])) {
                        $query->join('departments as d', 'pr.department_id', '=', 'd.id')
    ...
    public function export(Request $request) 
    {
        ...
        $callback = function() use ($viewMode, $request, $user) {
            ...
            $isGlobal = $user->hasRole(['Admin', 'Finance'])
                    || ($user->hasRole('Approver') && $user->site && $user->site->code === 'HO')
                    || $user->hasPrGlobalApproverAllSitesScope();
            ...
            if ($viewMode === 'items') {
                ...
                $itemQuery = \Modules\PrSystem\Models\PrItem::select([
                        'id',
                        'purchase_request_id',
                        'product_id',
                        'job_id',
                    ...
                    ->with([
                        'purchaseRequest:id,pr_number,request_date,user_id,department_id,sub_department_id,status',
                        'purchaseRequest.department:id,name',
                        'purchaseRequest.subDepartment:id,name',
                        'purchaseRequest.user:id,name',
                        'product:id,code,category',
                        'job:id,code,name',
                        'poItems:id,pr_item_id,purchase_order_id',
                        'poItems.purchaseOrder:id,po_number',
                    ])
                     ->whereHas('purchaseRequest', function($q) use ($applyVisibility) {
                         $applyVisibility($q);
                     });
                ...
                $itemQuery->chunkById(1000, function($items) use ($file) {
                ...
                $query->chunkById(500, function($prs) use ($file) {
                    foreach ($prs as $pr) {
                        ...
                        if ($pr->status === \Modules\PrSystem\Enums\PrStatus::PENDING->value || $pr->status === \Modules\PrSystem\Enums\PrStatus::ON_HOLD->value) {
                             ...
                             if ($nextApproval) {
                                  // Can show Role or User if assigned
                                    $approvalPos = $nextApproval->role_name ?? '-'; 
                                    if ($nextApproval->approver_id && $nextApproval->approver) {
    ...
    public function create()
    {
        ...
        $categories = config('prsystem.options.product_categories', []);
    ...
    public function store(Request $request)
    {
        ...
        if ($dept->budget_type === \Modules\PrSystem\Enums\BudgetingType::JOB_COA) {
            ...
            foreach ($itemsByJob as $jobId => $data) {
                ...
                if (($usedAmount + $amountNeeded) > $limit) {
                    $remaining = $limit - $usedAmount;
                    $warnings[] = "Budget Exceeded for Job '{$data['name']}'. Limit: ".number_format($limit).". Used: ".number_format($usedAmount).". Request: ".number_format($amountNeeded).". Remaining: ".number_format($remaining);
                }
            }
    ...
    public function show(PurchaseRequest $pr)
    {
        ...
        if ($pr->department->budget_type === \Modules\PrSystem\Enums\BudgetingType::JOB_COA) {
             ...
             foreach ($itemsByJob as $jobId => $data) {
                ...
                $budget = \Modules\PrSystem\Models\Budget::where('sub_department_id', $subDeptId)
                 ...
                 $limit = $budget ? $budget->totalAmount() : 0;
        ...
        if ($isJobCoa) {
            ...
            if ($jobId) {
                ...
                $budgetInfo['total'] = $budget ? $budget->totalAmount() : 0;
    ...
    public function getBudgetStatus($subDepartmentId)
    {
        ...
        if ($scope === 'job_coa') {
            $department = \Modules\PrSystem\Models\Department::find($subDepartmentId);
            if (!$department || $department->budget_type !== \Modules\PrSystem\Enums\BudgetingType::JOB_COA) {
                return response()->json([]);
            }

            $budgets = \Modules\PrSystem\Models\Budget::where('department_id', $department->id)
                ->where('year', $year)
                ->get();
            ...
            foreach ($budgets as $budget) {
                ...
                $status[$label] = [
                    'limit' => $budget->totalAmount(),
                    'used' => $usedAmount,
                    'remaining' => $budget->totalAmount() - $usedAmount,
                ];
            }
        ...
        $subDept = \Modules\PrSystem\Models\SubDepartment::with('department')->find($subDepartmentId);
        if ($subDept) {
            ...
            foreach ($categories as $cat) {
                $budgetAmount = isset($budgetsByKey[$cat]) ? $budgetsByKey[$cat]->totalAmount() : 0;
    ...
    public function getJobs($subDepartmentId)
    {
        ...
        $jobsByDirectBudget = DB::table('jobs')
            ...
            ->whereRaw('(COALESCE(budgets.amount, 0) + COALESCE(budgets.pta_amount, 0)) > 0')
    ...
    public function getJobsByDepartment(\Modules\PrSystem\Models\Department $department)
    {
        ...
        $budgetJobIds = \Modules\PrSystem\Models\Budget::query()
            ->where('year', $year)
                        ->whereRaw('(COALESCE(amount, 0) + COALESCE(pta_amount, 0)) > 0')
    ...
    {
        $filePath = $this->resolveAttachmentPath($purchaseRequest);
        if (!$filePath) {
    ...
    public function viewAttachment(PurchaseRequest $purchaseRequest)
    {
        $filePath = $this->resolveAttachmentPath($purchaseRequest);
        if (!$filePath) {
            return back()->with('error', 'File attachment tidak ditemukan.');
    ...
    private function resolveAttachmentPath(PurchaseRequest $purchaseRequest): ?string
    {
        $rawPath = trim((string) $purchaseRequest->attachment_path);
        ...
        foreach (['attachments/pr'] as $root) {
            if (!is_dir(storage_path('app/public/' . $root))) {
                continue;
            }

            foreach (Storage::disk('public')->allFiles($root) as $file) {
