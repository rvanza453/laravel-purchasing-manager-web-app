    ...
    <div class="card">
        <div class="card-header">
            <div class="flex justify-between items-center">
                <div>
                    <h3 class="text-lg font-bold">Konfigurasi Approver</h3>
                    <p class="text-sm text-gray-600">{{ $department->name }}</p>
                </div>
                <a href="{{ route('qc.approval-config.index') }}" class="btn btn-sm btn-secondary">
                    <i class="fas fa-arrow-left"></i> Kembali
                </a>
            </div>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('qc.approval-config.update', $department) }}">
                @csrf
                @method('PUT')
