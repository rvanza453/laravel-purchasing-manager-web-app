                ...
                <div class="grid">
                    ...
                    <div class="field">
                        ...
                        <select name="sub_department_id" id="sub_department_id" class="select" required>
                            ...
                            @if($finding->sub_department_id || old('sub_department_id'))
                                @php
                                    $subDeptId = old('sub_department_id', $finding->sub_department_id);
                ...
                <div id="panen-inspection-section" class="border-t border-gray-200 pt-6" style="display:none;">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                        <div>
                            ...
                            <input type="number" step="any" min="0" name="inspection_context[total_ha_block]" class="w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500" value="{{ old('inspection_context.total_ha_block', $finding->inspection_context['total_ha_block'] ?? '') }}" placeholder="34.91">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">SPH (Pokok/Ha)</label>
                            <input type="number" step="any" min="0" name="inspection_context[sph]" class="w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500" value="{{ old('inspection_context.sph', $finding->inspection_context['sph'] ?? '') }}" placeholder="108">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">ABW (Kg)</label>
                            <input type="number" step="any" min="0" name="inspection_context[abw]" class="w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500" value="{{ old('inspection_context.abw', $finding->inspection_context['abw'] ?? '') }}" placeholder="16">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">ALW (Kg)</label>
                            <input type="number" step="any" min="0" name="inspection_context[alw]" class="w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500" value="{{ old('inspection_context.alw', $finding->inspection_context['alw'] ?? '') }}" placeholder="0.08">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Nama Pemeriksa (QC)</label>
                            <input type="text" name="inspection_context[inspector_name]" class="w-full border-gray-300 rounded-md shadow-sm bg-gray-100 focus:border-primary-500 focus:ring-primary-500" value="{{ old('inspection_context.inspector_name', $finding->inspection_context['inspector_name'] ?? $authUser?->name ?? '-') }}" readonly>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Saksi Assistant</label>
                            <input type="text" name="inspection_context[assistant_witness]" class="w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500" value="{{ old('inspection_context.assistant_witness', $finding->inspection_context['assistant_witness'] ?? '') }}" placeholder="Nama saksi assistant">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Saksi Mandor</label>
                            <input type="text" name="inspection_context[mandor_witness]" class="w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500" value="{{ old('inspection_context.mandor_witness', $finding->inspection_context['mandor_witness'] ?? '') }}" placeholder="Nama saksi mandor">
                        </div>
                            ...
                            <div class="flex flex-col gap-2" style="display:flex; flex-direction:column; gap:8px;">
                                @foreach($finding->finding_attachments as $attachment)
                                    <div class="flex items-center gap-2" style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
                                        <a class="btn" href="{{ route('qc.findings.show', ['finding' => $finding, 'attachment_view' => $loop->index]) }}" target="_blank" style="width:fit-content;"><i class="fas fa-paperclip"></i> Lihat Lampiran {{ $loop->iteration }}</a>
                                        <label class="inline-flex items-center gap-2 text-sm text-red-600" style="cursor:pointer;">
                                            <input type="checkbox" name="finding_attachments_remove[]" value="{{ $attachment }}" class="attachment-remove-checkbox">
                                            Hapus
                                        </label>
                                    </div>
                                @endforeach
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const oldFindingItems = @json(old('finding_items', $finding->finding_items ?? []));
                ...
                if (btnAdd && pickerInput && finalInput && fileList) {
                    ...
                    document.querySelectorAll('.attachment-remove-checkbox').forEach((checkbox) => {
                        checkbox.addEventListener('change', function () {
                            if (this.checked) {
                                const row = this.closest('.flex.items-center.gap-2');
                                if (row) {
                ...
                async function loadAfdeling(departmentId) {
                    ...
                    try {
                        ...
                        const currentValue = afdSelect.value;
                        afdSelect.innerHTML = '<option value="">Pilih Afdeling</option>';
                        subDepts.forEach(sd => {
                            const option = document.createElement('option');
                            option.value = sd.id;
                            option.textContent = sd.name;
                            if (sd.id == currentValue) {
                                option.selected = true;
                            }
                ...
                if (deptSelect && afdSelect) {
                    ...
                    // Load afdelings on page load if department is already selected
                    if (deptSelect.value) {
                        loadAfdeling(deptSelect.value);
                    }
                }
