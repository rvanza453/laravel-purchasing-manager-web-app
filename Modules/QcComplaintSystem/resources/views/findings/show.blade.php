<x-qccomplaintsystem::layouts.master :title="'Detail Temuan QC'">
    <style>
        ...
        .deadline-pill.set { background: #dcfce7; color: #166534; }
        .deadline-pill.unset { background: #fee2e2; color: #b91c1c; }
    ...
    @endphp
    @php
        $inspectionContext = (array) ($finding->inspection_context ?? []);
        $findingItems = collect($finding->finding_items ?? []);
    @endphp
        ...
        <section class="hero">
            <div class="hero-top">
                ...
                <div class="pill-row">
                    ...
                    <span class="deadline-pill {{ $finding->target_resolution_date ? 'set' : 'unset' }}">
                        <i class="fas fa-calendar-day"></i>
                        {{ $finding->target_resolution_date ? 'Deadline Terisi' : 'Deadline Belum Terisi' }}
                    </span>
            ...
            <div class="meta-grid">
                ...
                <div class="meta-item">
                    <div class="label">Deadline Penyelesaian</div>
                    <div class="value">{{ optional($finding->target_resolution_date)->format('d M Y') ?? '-' }}</div>
                </div>
                <div class="meta-item">
                ...
                @if($finding->status === 'closed' && !is_null($closedLeadTimeDays))
                    <div class="meta-item">
                        <div class="label">Tanggal Diajukan</div>
                        <div class="value">{{ $submittedAt?->format('d M Y H:i') ?? '-' }}</div>
                    </div>
                    <div class="meta-item">
                        <div class="label">Closed Pada</div>
                        <div class="value">{{ $closedAt?->format('d M Y H:i') ?? '-' }}</div>
                    </div>
                    <div class="meta-item">
                        <div class="label">Lead Time Closed</div>
                        <div class="value">{{ $closedLeadTimeDays === 0 ? '< 1 hari' : $closedLeadTimeDays . ' hari' }}</div>
                    </div>
                @endif
            </div>
        </section>
        ...
        <section class="content-grid">
            <div class="section">
                ...
                <div class="section-body">
                    ...
                    @if(!empty(array_filter($inspectionContext)))
                        <div class="photo-box" style="margin-top:12px; background:#f8fcfa;">
                            <div class="k" style="margin-bottom:8px;">Informasi Kebun & Pemeriksaan</div>
                            <div class="kv"><div class="k">Total Ha Blok</div><div class="v">{{ $inspectionContext['total_ha_block'] ?? '-' }}</div></div>
                            <div class="kv"><div class="k">SPH (Pokok/Ha)</div><div class="v">{{ $inspectionContext['sph'] ?? '-' }}</div></div>
                            <div class="kv"><div class="k">ABW (Kg)</div><div class="v">{{ $inspectionContext['abw'] ?? '-' }}</div></div>
                            <div class="kv"><div class="k">ALW (Kg)</div><div class="v">{{ $inspectionContext['alw'] ?? '-' }}</div></div>
                            <div class="kv"><div class="k">Nama Pemeriksa (QC)</div><div class="v">{{ $inspectionContext['inspector_name'] ?? '-' }}</div></div>
                            <div class="kv"><div class="k">Saksi Assistant</div><div class="v">{{ $inspectionContext['assistant_witness'] ?? '-' }}</div></div>
                            <div class="kv" style="margin-bottom:0;"><div class="k">Saksi Mandor</div><div class="v">{{ $inspectionContext['mandor_witness'] ?? '-' }}</div></div>
                        </div>
                    @endif

                    @if($finding->kategori === 'panen' && $findingItems->isNotEmpty())
                        <div class="photo-box" style="margin-top:12px; background:#fffdf7;">
                            <div class="k" style="margin-bottom:8px;">Daftar Temuan</div>
                            <div style="overflow-x:auto;">
                                <table style="width:100%; border-collapse:collapse; font-size:13px;">
                                    <thead>
                                        <tr style="text-align:left; border-bottom:1px solid #dbe7e1;">
                                            <th style="padding:8px 6px; width:44px;">#</th>
                                            <th style="padding:8px 6px;">Nama Temuan</th>
                                            <th style="padding:8px 6px; width:120px;">Jumlah</th>
                                            <th style="padding:8px 6px; width:220px;">Catatan</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach($findingItems as $item)
                                            <tr style="border-bottom:1px solid #eef2f7;">
                                                <td style="padding:8px 6px;">{{ $loop->iteration }}</td>
                                                <td style="padding:8px 6px; font-weight:600;">{{ $item['label'] ?? $item['template_key'] ?? '-' }}</td>
                                                <td style="padding:8px 6px;">{{ isset($item['quantity']) && $item['quantity'] !== null && $item['quantity'] !== '' ? $item['quantity'] : '-' }}</td>
                            ...
                            <div class="flex flex-col gap-2" style="display:flex; flex-direction:column; gap:8px;">
                                @foreach($finding->finding_attachments as $attachment)
                                    <div style="display:flex; gap:8px; flex-wrap:wrap;">
                                        <a class="btn" href="{{ route('qc.findings.show', ['finding' => $finding, 'attachment_view' => $loop->index]) }}" target="_blank" style="width:fit-content; justify-content:flex-start;">
                                            <i class="fas fa-paperclip"></i> Lihat Lampiran {{ $loop->iteration }}
                                        </a>
                                        <a class="btn" href="{{ route('qc.findings.show', ['finding' => $finding, 'attachment_download' => $loop->index]) }}" style="width:fit-content; justify-content:flex-start;">
                            ...
                            @if($finding->completionEvidences->isNotEmpty())
                                <div style="display:grid; gap:8px;">
                                    @foreach($finding->completionEvidences as $evidence)
                                        <div style="display:flex; gap:8px; flex-wrap:wrap;">
                                            <a class="btn" href="{{ route('qc.findings.show', ['finding' => $finding, 'evidence_view' => $evidence->id]) }}" target="_blank" style="justify-content:flex-start;">
                                                <i class="fas fa-paperclip"></i>
                                                {{ $evidence->original_name }}
                                                <span class="text-muted" style="margin-left:auto; font-size:11px;">{{ number_format(($evidence->size ?? 0) / 1024, 0, ',', '.') }} KB</span>
                                            </a>
                                            <a class="btn" href="{{ route('qc.findings.show', ['finding' => $finding, 'evidence_download' => $evidence->id]) }}" style="justify-content:flex-start;">
                                                <i class="fas fa-download"></i> Download
                                            </a>
                                        </div>
                                    @endforeach
    ...
    @if($canSetDeadline && $finding->status !== 'closed')
        <div class="section" style="margin-top:14px;">
            <div class="section-head">Pengisian Deadline PIC</div>
            <div class="section-body">
                <div class="text-muted" style="margin-bottom:10px;">
                    Salah satu PIC wajib mengisi deadline agar progres penyelesaian lebih terukur dan upload bukti bisa dipercepat.
                </div>
                <form method="POST" action="{{ route('qc.findings.set-deadline', $finding) }}">
                    @csrf
                    <div class="grid">
                        <div class="field">
                            <label>Tanggal Deadline Penyelesaian</label>
                        ...
                        <div class="field">
                            <label>Rencana Tindak Lanjut (opsional)</label>
                            <input type="text" name="follow_up_plan" class="input" value="{{ old('follow_up_plan', $finding->follow_up_plan) }}" placeholder="Contoh: Perbaikan jalan dan dokumentasi ulang">
                        </div>
                    </div>
